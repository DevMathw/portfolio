            ...
            <div className="relative flex gap-6 px-6 py-3 rounded-full bg-white/80 backdrop-blur-lg shadow-xl dark:bg-darkTheme/80">
                {sections.map(({ id, label, icon: Icon }) => {
                    ...
                    return (
                        ...
                        <motion.div animate={{ scale: isActive ? 1.15 : 1 }} transition={{ type: 'spring', stiffness: 300 }} className={`p-2 rounded-full transition-colors ${ isActive ? 'text-green-400 dark:text-green-400' : 'text-gray-600 dark:text-gray-300' }`}>
                            <Icon size={22} />
                        </motion.div>
